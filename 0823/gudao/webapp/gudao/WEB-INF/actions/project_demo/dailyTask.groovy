...
Calendar now42 = Calendar.getInstance();
Calendar now43 = Calendar.getInstance();
Calendar now44 = Calendar.getInstance();
Calendar now45 = Calendar.getInstance();
...
now42.set(Calendar.DAY_OF_WEEK, Calendar.TUESDAY);
now43.set(Calendar.DAY_OF_WEEK, Calendar.WEDNESDAY);
now44.set(Calendar.DAY_OF_WEEK, Calendar.THURSDAY);
now45.set(Calendar.DAY_OF_WEEK, Calendar.FRIDAY);

Timestamp thisMonday = Timestamp.valueOf(sdf.format(now4.getTime()));
Timestamp thisTUESDAY = Timestamp.valueOf(sdf.format(now42.getTime()));
Timestamp thisWEDNESDAY = Timestamp.valueOf(sdf.format(now43.getTime()));
Timestamp thisTHURSDAY = Timestamp.valueOf(sdf.format(now44.getTime()));
Timestamp thisFRIDAY = Timestamp.valueOf(sdf.format(now45.getTime()));

/*//获取本周二日期
// now42.add(Calendar.DAY_OF_MONTH, -1); //解决周日会出现 并到下一周的情况?
now42.set(now42.get(Calendar.YEAR),now42.get(Calendar.TUESDAY),now42.get(Calendar.DAY_OF_MONTH), 1, 0, 0);
now42.set(Calendar.DAY_OF_WEEK, Calendar.TUESDAY); 
Timestamp thisTUESDAY = Timestamp.valueOf(sdf.format(now42.getTime()));

//获取本周三日期
// now43.add(Calendar.DAY_OF_MONTH, -1); //解决周日会出现 并到下一周的情况
now43.set(now43.get(Calendar.YEAR),now43.get(Calendar.WEDNESDAY),now43.get(Calendar.DAY_OF_MONTH), 1, 0, 0);
now43.set(Calendar.DAY_OF_WEEK, Calendar.WEDNESDAY); 
Timestamp thisWEDNESDAY = Timestamp.valueOf(sdf.format(now43.getTime()));

//获取本周四日期
// now44.add(Calendar.DAY_OF_MONTH, -1); //解决周日会出现 并到下一周的情况
now44.set(now44.get(Calendar.YEAR),now44.get(Calendar.THURSDAY),now44.get(Calendar.DAY_OF_MONTH), 1, 0, 0);
now44.set(Calendar.DAY_OF_WEEK, Calendar.THURSDAY); 
Timestamp thisTHURSDAY = Timestamp.valueOf(sdf.format(now44.getTime()));

//获取本周五日期
// now45.add(Calendar.DAY_OF_MONTH, -1); //解决周日会出现 并到下一周的情况
now45.set(now45.get(Calendar.YEAR),now45.get(Calendar.FRIDAY),now45.get(Calendar.DAY_OF_MONTH), 1, 0, 0);
now45.set(Calendar.DAY_OF_WEEK, Calendar.FRIDAY); 
Timestamp thisFRIDAY = Timestamp.valueOf(sdf.format(now45.getTime()));*/
...
context.thisMonday = thisMonday;
context.thisTuesday = thisTUESDAY;
context.thisTUESDAY = thisTUESDAY;
context.thisWednesday = thisWEDNESDAY;
context.thisWEDNESDAY = thisWEDNESDAY;
context.thisThursday = thisTHURSDAY;
context.thisTHURSDAY = thisTHURSDAY;
context.thisFriday = thisFRIDAY;
context.thisFRIDAY = thisFRIDAY;

context.now = dayOfWeek;
...
for (dailyTaskRecord in dailyTaskRecord6) {
    ...
    data.taskName = dailyTaskRecord.taskName;
    data.completedDate = dailyTaskRecord.completedDate;
    // Calendar now8 = dailyTaskRecord.completedDate;
    // dayOfWeek = now8.get(Calendar.DAY_OF_WEEK);
    // data.completedDate = dayOfWeek;
