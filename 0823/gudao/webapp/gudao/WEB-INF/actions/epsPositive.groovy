...
List<EntityCondition> pmCondsname = FastList.newInstance();
    ...
    pmCondsname.add(EntityCondition.makeCondition("statusId", EntityOperator.LIKE,  "%停产"));
    ...
    pmList = delegator.findList("ProductMasterResult", pmCond, null, ["finalEps DESC"], null, false); 
     for (pm in pmList) {	
   	if(pm.finalEps >= 0){
	    count++;
            plist.add(pm); 
