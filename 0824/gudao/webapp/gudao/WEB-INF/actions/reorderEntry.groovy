...
//context.d=distinctOwnerGroupList;
	...
	context.f = findReorderList;
			...
			for(ri in findReorderList){//loop datalist-start
				...
				//主供应商对应的起订量list
				List<GenericValue> mainMinimumOrderList = delegator.findByAnd("SupplierPrice",UtilMisc.toMap("productId",ri.productId,"supplier",supplierList.mainSupplier),["minimumOrderValue"],false);
				data.mainMin=mainMinimumOrderList.minimumOrderValue[0];
				data.mainCost=mainMinimumOrderList.unitPrice[0];
				//备供应商对应的起订量list
				List<GenericValue> backupMinimumOrderList = delegator.findByAnd("SupplierPrice",UtilMisc.toMap("productId",ri.productId,"supplier",supplierList.backupSupplier),["minimumOrderValue"],false);
