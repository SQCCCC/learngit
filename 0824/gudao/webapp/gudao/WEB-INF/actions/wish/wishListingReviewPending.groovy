...
expireDate = new java.sql.Date(nowDate.getTime() - 20 * 24 * 3600 * 1000);
...
conditions.add(EntityCondition.makeCondition("dateUploaded", EntityOperator.LESS_THAN, expireDate));
cond = EntityCondition.makeCondition(conditions, EntityOperator.AND);
List<GenericValue> allWishListingEndList = delegator.findList("WishListingReviewPending", cond, null, ["productStoreId" ,"wishId","sku"], null, false);
List<GenericValue> distinctStoreList = delegator.findList("WishListingReviewPending", null,  UtilMisc.toSet("productStoreId"), UtilMisc.toList("productStoreId"),
