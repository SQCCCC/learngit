...
gudaoTargetList = delegator.findList("GudaoTargetList", condition, null, ["period", "createdStamp DESC", "userLoginId"], null, false);

list =[];
for (gudaoTarget in gudaoTargetList) {
    data =[:];
    data.targetId = gudaoTarget.targetId;
    data.period = gudaoTarget.period;
    data.userLoginId = gudaoTarget.userLoginId;
    data.target = gudaoTarget.target;
    data.obstacles = gudaoTarget.obstacles;
    data.notes = gudaoTarget.notes;

    actionLists = delegator.findByAnd("GudaoTargetAction", UtilMisc.toMap("targetId", gudaoTarget.targetId), UtilMisc.toList("dueDate"), false);
    data.actionLists = actionLists;
