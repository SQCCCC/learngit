		    ...
		    // var result = 0;
		      ...
		      var a_${(order2.sku!)?replace('-','_')}=document.getElementById("selectKK_${(order2.sku!)?replace('-','_')}").value;
		      	if(a_${(order2.sku!)?replace('-','_')} == "MAIN") {
					...
					var counts_${(order2.sku!)?replace('-','_')} =  [];

					//<#list order2.minimumOrderCountListm as test>
					counts_${(order2.sku!)?replace('-','_')}.push(${test!});
					//</#list>

					var countsCostList_${(order2.sku!)?replace('-','_')} =  [];

					//var countsCostList_${(order2.sku!)?replace('-','_')} =  ${order2.minimumCostListm!};
					//<#list order2.minimumCostListm as test>
					countsCostList_${(order2.sku!)?replace('-','_')}.push(${test!});
					//</#list>	
					...
					//<#list order2.minimumOrderCountListb as test>
					counts_${(order2.sku!)?replace('-','_')}.push(${test!});
					//</#list>
					...
					//var countsCostList_${(order2.sku!)?replace('-','_')} =  ${order2.minimumCostListb!};
					//<#list order2.minimumCostListb as test>
					countsCostList_${(order2.sku!)?replace('-','_')}.push(${test!});
					//</#list>	
		      	}
			...
			var input11_${(order4.sku!)?replace('-','_')} = $("#input1_${(order4.sku!)?replace('-','_')}").val();
		      var a_${(order4.sku!)?replace('-','_')}=document.getElementById("selectKK_${(order4.sku!)?replace('-','_')}").value;
		      	if(a_${(order4.sku!)?replace('-','_')} == "MAIN") {
		      		//var counts_${(order4.sku!)?replace('-','_')} =  [];
					var counts_${(order4.sku!)?replace('-','_')} =  [];

					//<#list order4.minimumOrderCountListm as test>
					counts_${(order4.sku!)?replace('-','_')}.push(${test});
					//</#list>

					var countsCostList_${(order4.sku!)?replace('-','_')} =  [];


					//var countsCostList_${(order4.sku!)?replace('-','_')} =  ${order4.minimumCostListm!};
					//<#list order4.minimumCostListm as test>
					countsCostList_${(order4.sku!)?replace('-','_')}.push(${test});
					//</#list>	
					...
					//<#list order4.minimumOrderCountListb as test>
					counts_${(order4.sku!)?replace('-','_')}.push(${test});
					//</#list>
					...
					//var countsCostList_${(order4.sku!)?replace('-','_')} =  ${order4.minimumCostListb!};
					//<#list order4.minimumCostListb as test>
					countsCostList_${(order4.sku!)?replace('-','_')}.push(${test});
					//</#list>	
